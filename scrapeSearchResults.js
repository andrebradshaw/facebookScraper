...
function reger(x,el){	if(x.test(el) === true){	return x.exec(el)[0];	}else{		return '';		}}
...
function splitAt(exp,str,n){
	if(exp == "job"){
		...
		if(/^.+?(?= at )/.test(str) === false){ return str; }else{return job;}
	}
	if(exp == "co"){
		var co = grp(/(?<= at ).+/.exec(str),n); 
		if(/(?<= at ).+/.test(str) === false){ return str; }else{return co;}
	}
