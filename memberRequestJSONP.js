...
function cln(str){return str.replace(/\?/, "_QST_").replace(/\#/, "_HSH_").replace(/\&/, "_AMP_");}
var webAppUrl = "";
...
for(b=0; b<buttons.length; b++){
	...
	if(itext == "Approve"){
		...
		var output = "?fn="+cln(fullname)+"&url="+cln(linky)+"frn="+friends+"&gc="+groupConn+"&og="+otherGroup+"&liv="+cln(livesIn)+"&geo="+cln(geoLink)+"&jq="+cln(joinQ);
		window.open(webAppUrl+output);
	}
}
